...
const formatData = (session) => {
  return session.date.replace(/\b(\d{1,2}) (\w{3})\b/g, (match, day, month) => {
    const russianMonth = monthsObj[month];
    return russianMonth ? `${day} ${russianMonth}` : match;
  });
}
...
/*    setDetails(
    ...
    setDetails(formatData(sessions[index]))
  };
  //Load sessions info.
  useEffect(() => {
    let items = [];
