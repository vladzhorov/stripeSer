...
export const fillPersonalDetails = async (page: Page, name, email) => {
    ...
    await page.locator('[placeholder="Name"]').pressSequentially(name);
    // Change the focus
...
export const fillCardDetails = async (page: Page, card) => {
    ...
    await stripeIframe.getByLabel('Card number').pressSequentially(card, { delay: TYPE_DELAY(), timeout: 10000 });

    // Fill [placeholder="MM \/ YY"]
    await stripeIframe.getByLabel('Expiration').clear();
    await stripeIframe.getByLabel('Expiration').pressSequentially('04/30', { delay: TYPE_DELAY() });

    // Fill CVC field
    await stripeIframe.getByLabel('Security').clear();
    await stripeIframe.getByLabel('Security').pressSequentially('242', { delay: TYPE_DELAY() });

    // Fill [placeholder="ZIP"]
    await stripeIframe.getByLabel('ZIP').clear();
    await stripeIframe.getByLabel('ZIP').pressSequentially('42424', { delay: TYPE_DELAY() });
    // Change the focus
