...
test.describe('Updating account details', () => {
  ...
  test('Should not allow usage of existing Customer Email ID while Updating Account Info:5.1.4', async ({ page }) => {
    ...
    await page.locator('[placeholder="Email"]').pressSequentially(emailIdTest, { delay: TYPE_DELAY() });
  ...
  test('Should show Error Message if Invalid 3DS Card is used while Updating Account Info:5.1.7', async ({ page, request }) => {
    ...
    await page.waitForResponse((res) => {
      return res.url().includes('stripe.com');
    ...
    await expect(page.getByText('We are unable to authenticate your payment method. Please choose a different payment method and try again.')).toBeVisible({ timeout: 60 * 1000 });
  ...
  test('Should allow Customer to Successfully Update Payment after Card Decline:5.1.8', async ({ page, request }) => {
    ...
    await expect(page.locator('text=Your card has been declined.')).toBeVisible({ timeout: 10 * 1000 });
