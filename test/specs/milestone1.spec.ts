...
test.describe('Lesson signup form', () => {
  ...
  test('Should have Email and Name as Mandatory Fields:3.11', async ({ page }) => {
    ...
    await page.locator('[placeholder="Name"]').pressSequentially(faker.name.findName(), { delay: TYPE_DELAY() });
    ...
    await page.locator('[placeholder="Email"]').pressSequentially(faker.internet.email(), { delay: TYPE_DELAY() });
